...
sys.path.insert(1, '../lab3_part1_homeostat/')
...
def other_param_sweep_1D(n_units=4, n_runs=5, ks = np.linspace(0.1, 10, 20), do_plots=False, experiment=2):
    ...
    plt.xlabel("Needle spring damping")
    plt.ylabel("Average time units were adapting")
    plt.title("Parameter sweep over needle spring damping, with " + str(n_runs) + " runs")


# basic_analysis(n_runs=2, n_units=2, k=1, viability_scale=1, duration=1000, experiment=2)
# param_sweep_1D(n_units=4, n_runs=5, viability_scales=np.linspace(0.2, 1, 10), do_plots=True, experiment=0)
other_param_sweep_1D(n_units=4, n_runs=5, do_plots=False, ks=np.linspace(0.1, 10, 3), experiment=0)
