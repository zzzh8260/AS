...
class NoiseMaker(NoiseSource):
    ...
    def __init__(self,
        ...
        """
            ...
            :param white_noise_params: Parameters for the noisemaker's :class:`WhiteNoiseSource`. Defaults to [0, 0], in which case the noisemaker will produce no white noise.
            :type white_noise_params: list(float)

            :param brown_noise_step: The maximum step size for the noisemaker's :class`BrownNoiseSource`. Defaults to ``0``, in which case the noisemaker will produce no brown nosie.
            :type brown_noise_step: float

            :param spike_noise_params: The parameters for the noisemaker's :class:`SpikeNoiseSource`. Defaults to ``[0, 0, 0]``, in which case the noisemaker will produce no spike noise.
    ...
    def step(self, dt: float) -> float:
        ...
        """
        self.noise = 0.0
        for noise_source in self.noise_sources:  # for all noise sources, effects are accumulated (added)
    ...
    def reset(self):
        """
            Reset NoiseMaker, by resetting all of its noise sources.
        """
...
class GaussianNoiseSource(NoiseSource):
    """
        A class representing a source of gaussian noise.

        .. figure:: images/GaussianNoiseSource.svg
          :width: 400
          :align: center
          :alt: Gaussian noise
    """
    def __init__(self, mean: float, std: float):
        """
            __init__(self, mean: float, std: float)

            Construct instance of :class:`GaussianNoiseSource`.

            :param mean: The mean output from the :class:`GaussianNoiseSource`.
            :type mean: float

            :param std: The standard deviation of the nosie source.
            :type std: float
        """
        super().__init__()
        self.mean = mean
    ...
    def step(self, dt: float) -> float:
        """
            ...
            :rtype: float
        """
        self.noise = np.random.normal(self.mean, self.std)  # generate noise
        return super().step(dt)  # store noise
